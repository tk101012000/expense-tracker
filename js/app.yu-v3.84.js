...
function load() {
  ...
  // v3.58：旅程改為獨立帳本（txns 嵌在 trip 內），把舊版混在 DB.txns 的旅程交易遷移進去
  migrateTripsToIndependent();
...
/* ---------- 批量選擇（勾選刪除）狀態 ---------- */
let selMode = false;
let selType = null;          // 'txn' | 'bill'
const selSet = new Set();    // 已選取項目的 id
...
function enterSelMode(type) {
  selType = type; selMode = true; selSet.clear();
  if (type === 'txn') renderRecords(); else renderBills();
  $('#selBar').hidden = false;
  if (type === 'bill') $('#addBillBtn').hidden = true;
  $('#' + (type === 'txn' ? 'selRecordsBtn' : 'selBillsBtn')).classList.add('active');
  updateSelBar();
}
function exitSelMode() {
  selMode = false; selType = null; selSet.clear();
  $('#selBar').hidden = true;
  $('#addBillBtn').hidden = false;
  $('#selRecordsBtn').classList.remove('active');
  $('#selBillsBtn').classList.remove('active');
  if (currentView === 'records') renderRecords();
  else if (currentView === 'bills') renderBills();
}
function toggleSel(id) {
  if (selSet.has(id)) selSet.delete(id); else selSet.add(id);
  const el = document.querySelector(`[data-txn="${id}"],[data-billedit="${id}"]`);
  if (el) {
...
function getDisplayedIds() {
  const sel = selType === 'txn' ? '#recordsList [data-txn]' : '#billsList [data-billedit]';
  return Array.from(document.querySelectorAll(sel)).map(e => e.dataset.txn || e.dataset.billedit);
}
function refreshSelVisual() {
  document.querySelectorAll('#recordsList [data-txn], #billsList [data-billedit]').forEach(el => {
    const id = el.dataset.txn || el.dataset.billedit;
    const on = selSet.has(id);
    el.classList.toggle('selected', on);
    const chk = el.querySelector('.sel-check');
    if (chk) chk.classList.toggle('on', on);
...
function updateSelBar() {
  ...
  const all = ids.length > 0 && ids.every(id => selSet.has(id));
  $('#selCount').textContent = `已選 ${selSet.size} 項`;
  $('#selDelete').textContent = `刪除 (${selSet.size})`;
...
function selectAllToggle() {
  ...
  getDisplayedIds().forEach(id => on ? selSet.add(id) : selSet.delete(id));
  refreshSelVisual();
}
...
let billStatMonth = new Date().getMonth() + 1;   // 繳費統計月份選擇：預設當前月份；null=全年
...
function renderStats() {
  ...
  $('#categoryRank').innerHTML = cats.length ? cats.map((c, i) => {
    ...
    const barGradCSS = `linear-gradient(90deg, ${barColor}, ${barColor}99)`;
    return `<div class="rank-item">
      <span class="rank-label">${CAT_ICON[c.name] || ''} ${escapeHtml(c.name)}</span>
      <div class="rank-bar-wrap"><div class="rank-bar" style="width:${(c.value / total * 100).toFixed(1)}%;background:${barGradCSS}"></div></div>
      <span class="rank-val">${fmtMoney(c.value)} · ${(c.value / total * 100).toFixed(0)}%</span>
    </div>`;
  }).join('') : '<div class="empty">本月無支出</div>';
}
...
// 取得帳單的週期（缺失或空值預設 monthly，避免投影函數靜默跳過）
function billCycle(b) {
  return (b.cycle && b.cycle !== '') ? b.cycle : 'monthly';
}

function billAnnualCost(b) {
  if (!b.amount) return 0;
  const perYear = billCycle(b) === 'monthly' ? 12 : billCycle(b) === 'quarterly' ? 4 : 1;
...
function renderBillRank(list, elId, iconFn) {
  ...
  el.innerHTML = list.length ? list.map((c, i) => {
    ...
    return `<div class="rank-item">
      ...
      <div class="rank-bar-wrap"><div class="rank-bar" style="width:${pct}%;background:${barGradCSS}"></div></div>
...
function renderBillStats() {
  ...
  if (m) {
    ...
    // 統一用 cycle-adjusted 月金額（與大卡、圖表一致）
    const active = DB.bills.filter(b => {
      ...
      const perYear = billCycle(b) === 'monthly' ? 12 : billCycle(b) === 'quarterly' ? 4 : 1;
      return { ...b, monthCost: Math.round(b.amount * perYear / 12 * 100) / 100 };
    });
    $('#billDue').textContent = fmtMoney(active.reduce((s, b) => s + b.monthCost, 0));
    ...
    const top = active.map(b => ({ name: b.name, value: b.monthCost, icon: CAT_ICON[b.category] || '' }))
...
function drawBars(canvas, data) {
  ...
  data.forEach((d, i) => {
    ...
    const y = pad.t + ch - bh;
    // v3.48：漸層填充（品牌藍 → 淺藍）
    const barGrad = ctx.createLinearGradient(x, y + bh, x, y);
...
// v3.83：結算＝「把當下這一輪的帳結清」。標記已結算時，把該成員**目前計入的項目 key**
// 快照進 DB.paySettleSkip[成員id]，之後一律跳過 → 本次結算費用立即歸零，新記帳（新 key）從 0 重新累積。
// （v3.82 用時間戳比較會漏：未來到期的繳費、缺 createdAt 的舊資料都躲得過，故改為 ID 快照。）
function computePaySettle(period) {
  const items = [];
  // key：記帳用 txn:<id>；週期繳費用 bill:<id>:<dueDate>（含到期日，繳費滾到下一期會是新 key，才會重新計入）
  const push = (src, dateVal, key) => {
    ...
    items.push({ key, total: Number(src.amount) || 0, paidBy: src.paidBy || '', mode: src.splitMode, shares: src.shares });
  };
  DB.txns.forEach(t => push(t, t.date, 'txn:' + t.id));
  DB.bills.forEach(b => push(b, b.dueDate, 'bill:' + b.id + ':' + (b.dueDate || '')));
  if (!items.length) return null;

  const skip = DB.paySettleSkip || {};
  const skipOf = mid => skip[mid] || null;
  const fresh = (mid, key) => { const s = skipOf(mid); return !s || s.indexOf(key) === -1; };
  ...
  items.forEach(it => {
    ...
    amts.forEach(a => {
      ...
      if (!fresh(a.memberId, it.key)) return;
      map.get(a.memberId).payable += a.amount;
      addKey(a.memberId, it.key);
      counted = true;
    });
    if (it.paidBy && map.has(it.paidBy) && fresh(it.paidBy, it.key)) {
      map.get(it.paidBy).paid += it.total;
      addKey(it.paidBy, it.key);
      counted = true;
    }
  ...
  const rows = [...map.values()].filter(r => r.payable > 0 || r.paid > 0 || settledSet.has(r.id));
  ...
  return { rows, grand, itemCount, keysByMember: keysOut };
}

// 目前選擇的結算期間（結算快照要用同一個 period，否則會快照到別的範圍）
function payPeriodValue() {
  return $('#payPeriod') ? $('#payPeriod').value : 'all';
}

// v3.83 一次性遷移：v3.82 用時間戳制勾過「已結算」的成員，補一份 ID 快照，
// 讓「本次結算費用」在升版後真的被清掉（否則舊資料會一直算進去）。
function migratePaySettleSkip() {
  if (DB.__settleSkipMigrated) return;
  DB.__settleSkipMigrated = true;
  const settled = DB.paySettled || [];
  if (!settled.length) return;
  const skip = DB.paySettleSkip || (DB.paySettleSkip = {});
  if (Object.keys(skip).length) return;   // 已有快照 → 不用補
  const keys = (computePaySettle(payPeriodValue()) || {}).keysByMember || {};
  settled.forEach(m => { skip[m] = [...new Set([...(skip[m] || []), ...(keys[m] || [])])]; });
  save();
}
...
function renderPaySettle() {
  ...
  migratePaySettleSkip();   // v3.83：舊版（v3.82 時間戳制）已結算者補快照，只跑一次
...
function wirePaySettle() {
  ...
  markBtn.onclick = () => {
    ...
    const skip = DB.paySettleSkip || (DB.paySettleSkip = {});
    const now = Date.now();
    // 用「本次結算當下」的計算結果快照要排除的項目（同一 period）
    const snap = computePaySettle(payPeriodValue());
    const keys = (snap && snap.keysByMember) || {};
    let cleared = 0;
    pickedSet.forEach(m => {
      set.add(m);
      since[m] = now;
    ...
    pickedSet.clear();
    renderPaySettle();
    toast(`已結算 ${n} 位 · 清除 ${cleared} 筆本輪帳目，之後新增的記帳從 0 起算`);
  };
  resetBtn.onclick = () => {
    DB.paySettled = [];
    DB.paySettleSince = {};
...
const IMPORT_ALLOWED_TOP_KEYS = ['accounts', 'txns', 'bills', 'members', 'settings', 'trips'];
...
// v3.58：旅程為獨立帳本，trails.txns 內嵌每筆花費，匯入時需一併保留與清理
const TRIP_ALLOWED_FIELDS = ['id', 'name', 'destination', 'startDate', 'endDate', 'budget', 'currency', 'rates', 'note', 'memberIds', 'txns', 'createdAt'];
const TRIP_TXN_ALLOWED_FIELDS = ['id', 'type', 'amount', 'date', 'category', 'note', 'createdAt', 'paidBy', 'splitMode', 'shares', 'currency', 'accountId'];
...
function validateAndSanitize(parsed) {
  ...
  // v3.58：旅程（含獨立帳本 txns）需保留，否則匯入/雲端還原會清空所有旅程與其花費
  if (Array.isArray(parsed.trips)) {
    parsed.trips = parsed.trips.map(t => {
      const clean = sanitizeObj(t, TRIP_ALLOWED_FIELDS);
      if (!Array.isArray(clean.txns)) clean.txns = [];
      clean.txns = clean.txns.map(x => sanitizeObj(x, TRIP_TXN_ALLOWED_FIELDS));
      return clean;
    });
  } else {
    parsed.trips = [];
...
function bindListDelegation() {
  document.body.addEventListener('click', e => {
    // 批量選擇模式：點項目 = 切換勾選（不開編輯彈窗）
    if (selMode) {
      const item = e.target.closest('[data-txn],[data-billedit]');
      if (item && (item.dataset.txn ? selType === 'txn' : selType === 'bill')) {
        toggleSel(item.dataset.txn || item.dataset.billedit);
        return;
      }
    }
...
function bindSelEvents() {
  $('#selRecordsBtn').addEventListener('click', () => enterSelMode('txn'));
  $('#selBillsBtn').addEventListener('click', () => enterSelMode('bill'));
...
/* =========================================================
   旅遊記帳  yu-v3.58（與日常「記帳繳費」徹底獨立）
   旅程：DB.trips[] = { id, name, destination, startDate, endDate,
                        budget, currency, memberIds:[], rates:{}, note,
                        txns:[] }   ← 旅程交易獨立存放於 trip.txns，不進 DB.txns
   交易：trip.txns[] 內的每筆 = { id, type:'expense', amount, date, category,
                        note, currency, paidBy, splitMode, shares, createdAt }
   ========================================================= */
// v3.58：把舊版混在 DB.txns（帶 tripId 標籤）的旅程交易，搬進各自旅程的獨立帳本。
// 冪等：遷移後 DB.txns 不再含 tripId，故再次執行為 no-op。
function migrateTripsToIndependent() {
  DB.trips.forEach(t => { if (!Array.isArray(t.txns)) t.txns = []; });
  if (DB.txns.some(t => t && t.tripId)) {
    const keep = [];
    for (const t of DB.txns) {
      if (t && t.tripId) {
        const trip = DB.trips.find(x => x.id === t.tripId);
        if (trip) {
          const { tripId, ...rest } = t;   // 去掉 tripId 標籤，移入旅程帳本
          trip.txns.push(rest);
          continue;
        }
        delete t.tripId;                    // 旅程已不存在 → 轉為一般記帳，保留資料
      }
      keep.push(t);
    }
    DB.txns = keep;
...
// 旅程已花費（僅支出，依旅程幣別換算後合計；讀取旅程獨立帳本 trip.txns）
function tripSpent(trip) {
  const tcur = trip.currency || DB.settings.currency;
  return (trip.txns || []).filter(t => t.type === 'expense')
...
function openTripTxnModal(tripId, txnId) {
  ...
  if (!trip) return;
  editTripTxnTripId = tripId;
  editTripTxnId = txnId || null;
  ...
  if (txnId) {
    const t = (trip.txns || []).find(x => x.id === txnId);
    if (!t) { toast('該筆花費不存在'); modal.hidden = true; return; }
    $('#tripTxnAmount').value = t.amount;
    $('#tripTxnDate').value = t.date;
...
function saveTripTxn(e) {
  e.preventDefault();
  const trip = DB.trips.find(x => x.id === editTripTxnTripId);
  if (!trip) { toast('旅程不存在'); $('#tripTxnModal').hidden = true; render(); return; }
  ...
  if (editTripTxnId) {
    const t = trip.txns.find(x => x.id === editTripTxnId);
    if (!t) { toast('該筆花費不存在'); $('#tripTxnModal').hidden = true; return; }
    Object.assign(t, payload);
    toast('已更新花費');
  } else {
    trip.txns.push({ id: uid(), createdAt: Date.now(), ...payload });
    toast('已新增花費');
  }
  if (!save()) return;
  $('#tripTxnModal').hidden = true; render();
}
function deleteTripTxn(tripId, txnId) {
  const trip = DB.trips.find(x => x.id === tripId);
  if (!trip) return;
  const t = (trip.txns || []).find(x => x.id === txnId);
  showConfirm({
    title: '刪除花費', message: '確定要刪除這筆旅程花費嗎？此動作無法復原。', danger: true, confirmText: '刪除',
    context: t ? confirmRows([['金額', fmtMoney(t.amount), true], ['類別', t.category || '—'], ['日期', t.date || '—']]) : '',
    onConfirm: () => {
      trip.txns = (trip.txns || []).filter(x => x.id !== txnId);
      if (!save()) return;
      $('#tripTxnModal').hidden = true; toast('已刪除'); render();
    }
  });
}
...
function deleteTrip() {
  ...
  showConfirm({
    title: '刪除旅程',
    message: '確定要刪除這個旅程嗎？旅程內的 ' + (t ? tripTxns(t).length : 0) + ' 筆花費也會一併刪除，無法復原。',
    danger: true, confirmText: '刪除',
...
function bindTripsEvents() {
  ...
  // 旅程內花費彈窗
  const ttForm = $('#tripTxnForm');
  if (ttForm) ttForm.addEventListener('submit', saveTripTxn);
  const ttDel = $('#deleteTripTxnBtn');
...
window.BK = {
  ...
  importData: (str) => {
    ...
    DB.settings ||= {};
    DB.accounts ||= []; DB.txns ||= []; DB.bills ||= []; DB.members ||= []; DB.trips ||= [];
    migrateTripsToIndependent();  // v3.58：舊備份含 tripId 標籤交易時，遷移至旅程獨立帳本
    if (!save()) return;  // #8 修復
    render();
  },
